...
function BuffEvents:Constructor( regEvents )
    ...
    local effects = self.playerService.player:GetEffects();
	for i = 1, effects:GetCount() do
		local effect = effects:Get(i);
		local name = effect:GetName();
		
		if ( self.registeredEvents.classes[ self.playerClass ] ~= nil and self.registeredEvents.classes[ self.playerClass ].effects ~= nil and self.registeredEvents.classes[ self.playerClass ].effects[name] ~= nil ) then
			self.registeredEvents.classes[ self.playerClass ].effects[name].count = self.registeredEvents.classes[ self.playerClass ].effects[name].count + 1;
		end
	...
	self.eventService:AddCallback( effects, "EffectAdded", function(sender, args)
		local effects = self.playerService.player:GetEffects()
		local effect = effects:Get(args.Index);

		local name = effect:GetName();
		if ( self.registeredEvents.classes[ self.playerClass ] ~= nil and self.registeredEvents.classes[ self.playerClass ].effects[name] ~= nil ) then
			self.registeredEvents.classes[ self.playerClass ].effects[name].count = self.registeredEvents.classes[ self.playerClass ].effects[name].count + 1;
		end
	...
	self.eventService:AddCallback( effects, "EffectRemoved", function(sender, args)
	...
	self.eventService:AddCallback( effects, "EffectsCleared", function(sender, args)
		local effect = args.Effect;

		local name = effect:GetName();
		if ( self.registeredEvents.classes[ self.playerClass ].effects[name] ~= nil ) then
			self.registeredEvents.classes[ self.playerClass ].effects[name].count = self.registeredEvents.classes[ self.playerClass ].effects[name].count - 1;
		end
...
function BuffEvents:CheckVisibility( barSettings )
	...
	if (barSettings.events.effects ~= nil ) then
		if ( barSettings.events.triggerOnClassBuffActive == true ) then
			for key2, value2 in pairs (self.registeredEvents.classes[ self.playerClass ].effects) do
