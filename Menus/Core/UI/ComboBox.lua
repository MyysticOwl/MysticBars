...
function ComboBox:Constructor()
    ...
    self.dropped = false;
    self.selection = -1;
    self.numberOfVisibleOptions = 10;
...
function ComboBox:ShowDropDown()
    ...
    if ((itemCount > 0) and not (self.dropped)) then
        ...
        local scrollSize = 0;
        if (maxItems > self.numberOfVisibleOptions) then
            maxItems = self.numberOfVisibleOptions;
            scrollSize = self.numberOfVisibleOptions;
        end
...
function ComboBox:SetNumberOfVisibleOptions(numberOfOptions)
